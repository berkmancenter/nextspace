...
/* `foldedFrom` is ahead of the generated spec: the backend added it for concept folding (a
   low-connection concept consolidated into a related one once a series graph grows past its
   size cap) before this ran against a deployed spec that has caught up. Narrowed here the
   same way ArtifactVersion below narrows what the spec leaves optional, rather than by
...
/* The backend defaults every array to [], so a payload never lacks one. `concepts` is
   narrowed to the extended GraphConcept above (foldedFrom), same reason as there. */
