...
import { Api, GetChannelPasscode, QueryParamsError, SendData, emitWithTokenRefresh } from '../utils';
...
function BackchannelRoom({ authType }: { authType: AuthType }) {
  ...
  const [transcriptPasscode, setTranscriptPasscode] = useState<string | null>(null);
  ...
  const { socket, pseudonym, isConnected, errorMessage: sessionError } = useSessionJoin();
  ...
  useEffect(() => {
    ...
    async function fetchConversationData() {
      if (router.query.channel) {
        setTranscriptPasscode(GetChannelPasscode('transcript', router.query, setLocalError));
        const participantPasscodeParam = GetChannelPasscode('participant', router.query, setLocalError);
  ...
  function sendMessage(message: string, preset = false) {
    ...
    if (preset) {
      trackConversationEvent(conversationId, 'backchannel', 'quick_response_sent', message);
    } else {
      trackConversationEvent(conversationId, 'backchannel', 'custom_message_sent', 'custom_message');
    }
  ...
  const QuickResponseButton: FC<{ label: string; icon: ReactNode }> = ({ label, icon }) => (
    <Button
      onClick={async () => {
        ...
        <div className="w-11/12 lg:w-2/3">
          {showWelcome ? (
            <div className="flex flex-col h-screen items-center">
              <h1 className="text-xl font-bold text-medium-slate-blue text-center">Welcome to your backchannel</h1>
              <div className="mt-4 mx-2 lg:mx-28 text-lg text-gray-500">
                <p>
                  This is the LLM backchannel, here to help today&apos;s moderator steer the discussion toward what the
                  audience cares about most.
                </p>
                <p className="mt-4">
                  Send reactions, questions, or topics you want more of, and the moderator will get a digestible summary.
                </p>
                <p className="mt-4">
                  Please note that a pseudonymized message transcript will be visible to our engineering team. Please share
                  your feedback on the tool at <b>brk.mn/feedback!</b>, and enjoy the event!
                </p>
              </div>
              <Button
                variant="contained"
                ...
                onClick={() => {
                  ...
                  const conversationId = router.query.conversationId as string;
                  if (conversationId) {
                    trackConversationEvent(conversationId, 'backchannel', 'welcome_dismissed', 'welcome_screen');
                  }
                }}
            ...
            <>
              <div className="max-w-2.5" aria-label={`Connection status is ${isConnected ? 'connected' : 'disconnected'}`}>
                {isConnected ? (
                  <Tooltip
                    title={
                      <>
              ...
              <Box display="flex" flexDirection="column" height="90vh" overflow="hidden">
                {/* Conversation View */}
                <Box flex="1" overflow="auto" display="flex" flexDirection="column" gap="8px" id="scroll-container">
                  {messages.map((message, i) => (
                    <BackchannelMessage key={`msg-${i}`} message={{ ...message, body: (message.body as any).text }} />
                  ))}
                  ...
                  <div className="w-11/12 lg:w-4/5">
                    ...
                    <Box display="flex" alignItems="center" padding="8px">
                      <TextField
                        ...
                        onKeyUp={(e) => {
                          if (e.key === 'Enter') sendMessage(messageInputRef.current!.value);
                        }}
                        style={{
                          flex: 1,
                          padding: '8px',
                          ...
                          input: {
                            endAdornment: (
                              <InputAdornment position="end">
                                <IconButton
                                  onClick={() => sendMessage(messageInputRef.current!.value)}
                                  aria-label="send message"
                                  disabled={!currentMessage || currentMessage.length < 1}
                                >
                                  <Send
                                    sx={{
                                      color: !currentMessage || currentMessage.length < 1 ? 'grey' : '#4845D2',
                                    }}
                                  />
                                </IconButton>
                              </InputAdornment>
