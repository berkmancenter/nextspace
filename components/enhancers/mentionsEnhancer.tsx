...
import { InputEnhancer, TriggerMatch, InputChangeRequest } from '../../types/inputEnhancer';
 ...
 */
export const createMentionsEnhancer = (contributors: string[]): InputEnhancer<MentionItem> => ({
  id: 'mentions',

  button: {
    icon: '@',
    getTitle: (isActive) => (isActive ? 'Remove @ mention' : 'Insert @ to mention someone'),
  ...
  getItems: (query): MentionItem[] => {
    return contributors.filter((p) => p.toLowerCase().startsWith(query.toLowerCase())).map((p) => ({ pseudonym: p }));
  },
