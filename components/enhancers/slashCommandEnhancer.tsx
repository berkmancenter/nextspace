 ...
 */
export const createSlashCommandEnhancer = (commands: SlashCommand[]): InputEnhancer<SlashCommand> => ({
  id: 'slash-commands',

  button: {
    icon: '/',
    getTitle: (isActive) => (isActive ? 'Remove / command' : 'Insert / to show commands'),
  ...
  getItems: (query): SlashCommand[] => {
    return commands.filter((cmd) => cmd.command.toLowerCase().startsWith(query.toLowerCase()));
  },
