...
export const MessageFeedback: FC<MessageFeedbackProps> = ({
  ...
  const [selectedFeedback, setSelectedFeedback] = useState<string | null>(initialRating || null);
  ...
  const handleFeedbackClick = (feedbackText: string) => {
    if (selectedFeedback !== null || !messageId || !onSendFeedbackRating) return;
  ...
  return (
    <Box display="flex" flexDirection="column" gap="0.5rem" marginTop="0.25rem">
      ...
      {/* Header Row */}
      <Typography variant="caption" id="rating-group-label" className="text-gray-600">
        How did the bot do?
      </Typography>
      {/* Feedback Buttons Row */}
      <Box display="flex" alignItems="center" gap="0.5rem" role="radiogroup" aria-labelledby="rating-group-label">
        {feedbackOptions.map((option, index) => (
          <Button
            ...
            tabIndex={selectedFeedback !== null ? -1 : focusedButton === index ? 0 : -1}
            className={selectedFeedback === option.text ? 'border border-primary text-primary' : 'border text-primary'}
            sx={{
              minWidth: '44px',
              height: '30px',
              paddingX: '10px',
              borderRadius: '5px',
              borderWidth: '1px',
              borderColor: selectedFeedback === option.text ? undefined : '#666666',
              backgroundColor: (theme) =>
                selectedFeedback === option.text ? alpha(theme.palette.primary.main, 0.08) : 'white',
              textTransform: 'none',
              fontSize: '0.6875rem',
              fontWeight: 700,
              '&:hover': {
