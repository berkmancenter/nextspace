...
describe('NavigationBar', () => {
  ...
  it("calls onTabChange with 'resources' when resources tab clicked", async () => {
    ...
    render(<NavigationBar {...baseProps} showResources={true} onTabChange={onTabChange} />);
  ...
  it("marks resources tab as active with aria-current='page' when activeTab is resources", () => {
    render(<NavigationBar {...baseProps} showResources={true} activeTab="resources" />);
  ...
  it("marks the active tab with aria-current='page'", () => {
    ...
    const inactiveBtns = screen.getAllByLabelText('Group Chat').filter((btn) => btn.getAttribute('aria-current') === 'page');
  ...
  it('applies active background style to the active tab button', () => {
    ...
    const activeChatBtns = screen
      ...
      .filter((btn) => (btn as HTMLElement).style.background === 'rgb(209, 196, 233)');
    expect(activeChatBtns.length).toBeGreaterThan(0);
  });

  it('applies bold font-weight label to the active tab', () => {
    const { container } = render(<NavigationBar {...baseProps} activeTab="assistant" />);
  ...
  describe('unseen badges', () => {
    it('does not show badge when unseenChatCount is 0', () => {
      const { container } = render(<NavigationBar {...baseProps} unseenChatCount={0} activeTab="assistant" />);
    ...
    it('shows badge on chat tab when unseenChatCount > 0 and not on chat tab', () => {
      const { container } = render(<NavigationBar {...baseProps} unseenChatCount={3} activeTab="assistant" />);
    ...
    it('shows badge on resources tab when unseenResourcesCount > 0 and not on resources tab', () => {
      const { container } = render(
        <NavigationBar {...baseProps} showResources={true} unseenResourcesCount={3} activeTab="assistant" />,
      );
    ...
    it('hides badge on resources tab when active even with unseenResourcesCount > 0', () => {
      const { container } = render(
        <NavigationBar {...baseProps} showResources={true} unseenResourcesCount={3} activeTab="resources" />,
      );
    ...
    it('shows badge when on chat tab with unseenChatCount but no unreadChatReplyCount', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenChatCount={3} unreadChatReplyCount={0} activeTab="chat" />,
      );
  ...
  describe('unreadChatReplyCount badge logic', () => {
    it('shows badge on chat tab when unreadChatReplyCount > 0 even when chat tab is active', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenChatCount={0} unreadChatReplyCount={2} activeTab="chat" />,
      );
    ...
    it('shows badge on chat tab when unseenChatCount > 0 and chat tab is not active', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenChatCount={3} unreadChatReplyCount={0} activeTab="assistant" />,
      );
    ...
    it('shows badge on chat tab when both unseenChatCount and unreadChatReplyCount > 0 and chat tab is active', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenChatCount={5} unreadChatReplyCount={2} activeTab="chat" />,
      );
    ...
    it('hides badge on chat tab when both unseenChatCount and unreadChatReplyCount are 0 and chat tab is active', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenChatCount={0} unreadChatReplyCount={0} activeTab="chat" />,
      );
    ...
    it('does not show badge on chat tab when only unreadChatReplyCount > 0 and not on chat tab', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenChatCount={0} unreadChatReplyCount={3} activeTab="assistant" />,
      );
    ...
    it('special behavior: chat tab shows badge even when active if unreadChatReplyCount > 0', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenChatCount={0} unreadChatReplyCount={1} activeTab="chat" />,
      );
    ...
    it('handles the condition: (unseen > 0 || (isActive && unreadChatReplyCount > 0))', () => {
      ...
      const { container: container1, unmount: unmount1 } = render(
        <NavigationBar {...baseProps} unseenChatCount={1} unreadChatReplyCount={0} activeTab="assistant" />,
      );
      ...
      const { container: container2, unmount: unmount2 } = render(
        <NavigationBar {...baseProps} unseenChatCount={0} unreadChatReplyCount={3} activeTab="chat" />,
      );
      badges = container2.querySelectorAll('.MuiBadge-badge');
      visibleBadges = Array.from(badges).filter((badge) => !(badge as HTMLElement).classList.contains('MuiBadge-invisible'));
      expect(visibleBadges.length).toBeGreaterThan(0);
      unmount2();
      ...
      const { container: container4 } = render(
        <NavigationBar {...baseProps} unseenChatCount={0} unreadChatReplyCount={0} activeTab="assistant" />,
      );
  ...
  describe('unreadAssistantReplyCount badge logic', () => {
    it('shows badge on assistant tab when unreadAssistantReplyCount > 0 even when assistant tab is active', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenAssistantCount={0} unreadAssistantReplyCount={2} activeTab="assistant" />,
      );
    ...
    it('shows badge on assistant tab when unseenAssistantCount > 0 and assistant tab is not active', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenAssistantCount={3} unreadAssistantReplyCount={0} activeTab="chat" />,
      );
    ...
    it('shows badge on assistant tab when both unseenAssistantCount and unreadAssistantReplyCount > 0 and assistant tab is active', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenAssistantCount={5} unreadAssistantReplyCount={2} activeTab="assistant" />,
      );
    ...
    it('hides badge on assistant tab when both unseenAssistantCount and unreadAssistantReplyCount are 0 and assistant tab is active', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenAssistantCount={0} unreadAssistantReplyCount={0} activeTab="assistant" />,
      );
    ...
    it('does not show badge on assistant tab when only unreadAssistantReplyCount > 0 and not on assistant tab', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenAssistantCount={0} unreadAssistantReplyCount={3} activeTab="chat" />,
      );
    ...
    it('special behavior: assistant tab shows badge even when active if unreadAssistantReplyCount > 0', () => {
      const { container } = render(
        <NavigationBar {...baseProps} unseenAssistantCount={0} unreadAssistantReplyCount={1} activeTab="assistant" />,
      );
