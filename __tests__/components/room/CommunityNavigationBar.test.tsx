...
describe('CommunityNavigationBar', () => {
  it('renders the Group Chat and Berkie tabs', () => {
    render(<CommunityNavigationBar activeTab="chat" onTabChange={jest.fn()} unreadAssistantCount={0} />);
  ...
  it('calls onTabChange with the tab id when a tab is clicked', async () => {
    ...
    render(<CommunityNavigationBar activeTab="chat" onTabChange={onTabChange} unreadAssistantCount={0} />);
  ...
  it('states the unread count in the Berkie tab aria-label when there are unread messages', () => {
    render(<CommunityNavigationBar activeTab="chat" onTabChange={jest.fn()} unreadAssistantCount={2} />);
  ...
  it('uses the singular form for exactly one unread message', () => {
    render(<CommunityNavigationBar activeTab="chat" onTabChange={jest.fn()} unreadAssistantCount={1} />);
  ...
  it('does not show the unread aria-label on the Berkie tab while it is active', () => {
    render(<CommunityNavigationBar activeTab="assistant" onTabChange={jest.fn()} unreadAssistantCount={3} />);
    expect(screen.getByRole('button', { name: 'Berkie' })).toBeInTheDocument();
  });

  it('has no accessibility violations', async () => {
    const { container } = render(
      <CommunityNavigationBar activeTab="chat" onTabChange={jest.fn()} unreadAssistantCount={2} />,
    );
