...
describe('Analytics Utility', () => {
  ...
  describe('when analytics is disabled', () => {
    ...
    it('should log info message about disabled analytics in development', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Analytics is disabled'));
  ...
  describe('when analytics is enabled but URL is missing', () => {
    ...
    it('should warn about missing MATOMO_URL', async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining('NEXT_PUBLIC_MATOMO_URL environment variable is not set'),
      );
    });

    it('should only warn once even with multiple tracking calls', async () => {
      const { trackPageView, trackEvent } = await import('../../utils/analytics');
  ...
  describe('when analytics is enabled with URL configured', () => {
    ...
    it('should initialize without warnings when URL is configured', async () => {
      ...
      expect(consoleWarnSpy).not.toHaveBeenCalledWith(expect.stringContaining('NEXT_PUBLIC_MATOMO_URL'));
    });
  });
  ...
  describe('trackConversationEvent', () => {
    ...
    it('should set conversation_id dimension and track event', async () => {
      ...
      trackConversationEvent('conv-123', 'assistant', 'message_sent', 'question');
      ...
      expect((global.window as any)._paq[0]).toEqual(['setCustomDimension', 6, 'conv-123']);

      // Second call: trackEvent
      expect((global.window as any)._paq[1]).toEqual(['trackEvent', 'assistant', 'message_sent', 'question', undefined]);
    });

    it('should track with event name', async () => {
      const { trackConversationEvent } = await import('../../utils/analytics');

      trackConversationEvent('conv-456', 'moderator', 'metrics_clicked', 'metrics_panel');

      // Both dimension and event should be in _paq
      expect((global.window as any)._paq.length).toBe(2);
      expect((global.window as any)._paq[0]).toEqual(['setCustomDimension', 6, 'conv-456']);
    ...
    it("should track message_sent with 'message' for typed messages", async () => {
      ...
      trackConversationEvent('conv-789', 'assistant', 'message_sent', 'message');

      // Both dimension and event should be in _paq
      expect((global.window as any)._paq.length).toBe(2);
      expect((global.window as any)._paq[0]).toEqual(['setCustomDimension', 6, 'conv-789']);
      expect((global.window as any)._paq[1]).toEqual(['trackEvent', 'assistant', 'message_sent', 'message', undefined]);
    });

    it("should track message_sent with 'reaction' for prompt selections", async () => {
      const { trackConversationEvent } = await import('../../utils/analytics');

      trackConversationEvent('conv-789', 'assistant', 'message_sent', 'reaction');

      // Both dimension and event should be in _paq
      expect((global.window as any)._paq.length).toBe(2);
      expect((global.window as any)._paq[0]).toEqual(['setCustomDimension', 6, 'conv-789']);
      expect((global.window as any)._paq[1]).toEqual(['trackEvent', 'assistant', 'message_sent', 'reaction', undefined]);
    });
    ...
    it('should track rating_submitted separately from message_sent', async () => {
      ...
      // Both dimension and event should be in _paq
      expect((global.window as any)._paq.length).toBe(2);
      expect((global.window as any)._paq[0]).toEqual(['setCustomDimension', 6, 'conv-101']);
      expect((global.window as any)._paq[1]).toEqual(['trackEvent', 'assistant', 'rating_submitted', '5', undefined]);
    });

    it('should track feedback_sent separately from message_sent', async () => {
      const { trackConversationEvent } = await import('../../utils/analytics');

      trackConversationEvent('conv-102', 'assistant', 'feedback_sent', 'helpful');

      // Both dimension and event should be in _paq
      expect((global.window as any)._paq.length).toBe(2);
      expect((global.window as any)._paq[0]).toEqual(['setCustomDimension', 6, 'conv-102']);
      expect((global.window as any)._paq[1]).toEqual(['trackEvent', 'assistant', 'feedback_sent', 'helpful', undefined]);
    });
    ...
    it('should track command_sent for slash commands', async () => {
      ...
      // Both dimension and event should be in _paq
      expect((global.window as any)._paq.length).toBe(2);
      expect((global.window as any)._paq[0]).toEqual(['setCustomDimension', 6, 'conv-103']);
      expect((global.window as any)._paq[1]).toEqual(['trackEvent', 'assistant', 'command_sent', 'mod', undefined]);
    });

    it('should track command_sent for system commands', async () => {
      const { trackConversationEvent } = await import('../../utils/analytics');

      trackConversationEvent('conv-104', 'assistant', 'command_sent', 'feedback');

      // Both dimension and event should be in _paq
      expect((global.window as any)._paq.length).toBe(2);
      expect((global.window as any)._paq[0]).toEqual(['setCustomDimension', 6, 'conv-104']);
