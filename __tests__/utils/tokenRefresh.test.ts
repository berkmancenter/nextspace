...
import { ensureFreshToken, refreshAccessToken, emitWithTokenRefresh } from '../../utils/tokenRefresh';
...
describe('tokenRefresh utilities', () => {
  ...
  describe('emitWithTokenRefresh', () => {
    ...
    it('emits the event with the fresh access token in the data payload', async () => {
      ...
      mockSocket.emit.mockImplementationOnce((_event: string, _data: any, callback: (r: any) => void) => {
        callback({ success: true });
      });

      const eventData = { conversationId: 'conv-1', token: 'old-token' };

      await emitWithTokenRefresh(mockSocket, 'conversation:join', eventData, onSuccess, onError);
    ...
    it('retries with refreshed token when socket returns a 401 auth error', async () => {
      ...
      await emitWithTokenRefresh(mockSocket, 'conversation:join', { token: 'old-token' }, onSuccess, onError);
    ...
    it('passes non-auth errors directly to onError without retrying', async () => {
      ...
      await emitWithTokenRefresh(mockSocket, 'conversation:join', { token: 'valid-token' }, onSuccess, onError);
