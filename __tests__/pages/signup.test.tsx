...
describe('CreateAccountPage', () => {
  ...
  it('validates password requirements', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Password must be at least 8 characters and contain at least one letter and one number/i),
      ).toBeInTheDocument();
    });
  ...
  it('successfully submits valid form data', async () => {
    ...
    await user.type(screen.getByLabelText(/Re-enter Password/i), 'ValidPass123');
    ...
    await waitFor(() => {
      expect(screen.getByText('Account created successfully! Sending you to login.')).toBeInTheDocument();
    });
  ...
  it('handles registration errors gracefully', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Username or email already exists. Please choose another.')).toBeInTheDocument();
    });
  ...
  it('handles network errors during registration', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Failed to register user.*Network error/i)).toBeInTheDocument();
    });
  });
  ...
  it('handles pseudonym retrieval failure', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Problem retrieving pseudonym')).toBeInTheDocument();
    });
