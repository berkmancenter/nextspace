...
describe('GuidePage', () => {
  ...
  it('fetches from the correct endpoint', async () => {
    ...
    render(<GuidePage />);
    await waitFor(() =>
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3000/v1/conversations/test-conv-id/features'),
    );
  });
  ...
  it('shows caveat when a slash command is disabled', async () => {
    resolveWith(GUIDE_RESPONSE_WITH_DISABLED_SLASH);
    render(<GuidePage />);
    await waitFor(() => expect(screen.getByText(/feature availability varies by event/i)).toBeInTheDocument());
  });
